...
mysqli_select_db($saha, $database_saha);$query_selected = "SELECT * FROM crawl WHERE crawl.deleted='N'  AND crawl.block_update='N' AND crawl.crawlRunImages='Y' ORDER BY crawl.id DESC LIMIT $BatchCount";
...
mysqli_select_db($saha, $database_saha);$query_settings = "SELECT * FROM crawl WHERE crawl.deleted='N'  AND crawl.block_update='N' AND crawl.crawlRunImages='Y' ORDER BY crawl.id DESC LIMIT $BatchCount";
$settings = mysqli_query($saha, $query_settings) or die(mysqli_error($saha));
$row_settings = mysqli_fetch_assoc($settings);
$totalRows_settings = mysqli_num_rows($settings);
...
mysqli_select_db($saha, $database_saha);$query_CrawlIPending = "SELECT * FROM crawl WHERE crawl.crawlRunImages='Y'";
...
ajaxFunctionf<?php echo $coluMs2[$i+1];  ?>('load<?php echo $coluMs2[$i+1];  ?>','auto','load<?php echo $coluMs2[$i+1];  ?>','','','admin-crawl-step2-f.php','<?php echo $coluMs2[$i+1];  ?>');
...
mysqli_select_db($saha, $database_saha);$query_selectedCrawl = "SELECT * FROM crawl_images WHERE crawl_images.current_url= '$CURL'";
...
?>                 
                                                ...
                                                <tr>
                                                   <td><a href="<?php echo $row_selected['current_url']; ?>" target="_blank" class="client-link" data-toggle="tab"><?php echo $row_selected['current_url']; ?></a>
