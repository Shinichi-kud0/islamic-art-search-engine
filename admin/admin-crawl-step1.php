...
mysqli_select_db($saha, $database_saha);$query_settings = "SELECT * FROM settings ORDER BY settings.id DESC";
...
mysqli_select_db($saha, $database_saha);$query_settingsCrawl = "SELECT * FROM crawl_settings";
...
mysqli_select_db($saha, $database_saha);$query_selected = "SELECT * FROM settings WHERE settings.crawlRun='Y' ORDER BY settings.id DESC LIMIT $BatchCount";
...
mysqli_select_db($saha, $database_saha);$query_settings = "SELECT * FROM settings WHERE settings.crawlRun='Y' ORDER BY settings.id DESC LIMIT $BatchCount";
$settings = mysqli_query($saha, $query_settings) or die(mysqli_error($saha));
$row_settings = mysqli_fetch_assoc($settings);
$totalRows_settings = mysqli_num_rows($settings);

if($totalRows_settings==0){
	header("Location:admin-crawl-step2.php");
	exit;
}

mysqli_select_db($saha, $database_saha);$query_CrawlIPending = "SELECT * FROM settings WHERE settings.crawlRun='Y'";
...
ajaxFunctionf<?php echo $coluMs2[$i+1];  ?>('load<?php echo $coluMs2[$i+1];  ?>','auto','load<?php echo $coluMs2[$i+1];  ?>','','','admin-crawl-step1-f.php','<?php echo $coluMs2[$i+1];  ?>');
...
ajaxFunctionf<?php echo $coluMs[0];  ?>('load<?php echo $coluMs[0];  ?>','auto','load<?php echo $coluMs[0];  ?>','','','admin-crawl-step1-f.php','<?php echo $coluMs[0];  ?>');
...
function CrawlingStatus() {
    if(document.getElementById('CrawlDown').innerHTML=="Yes"){
		...
		window.location='admin-crawl-step1.php';
	}else{
...
CRAWLING IN PROGRESS - DATA (Step 01 of 03)<br>   <br>
All Pedning Sites / URL for Link Crawling : $totalRows_CrawlIPending 
...
<strong style=\"background:#000; color:#FFF; padding:10px;\"> Batch Count : $BatchCount</strong>

<br><br>

<strong style=\"background:#000; color:#FFF; padding:10px;\"> Max Links per Site/URL : $MaxLinksPerSite </strong>
<br><br>
...
?>
						...
						<table class="table table-striped table-hover">
                                          ...
                                          <tr>
                                                  ...
                                                  <td>Total Visits</td>
                                                  <td class="client-status">Indexed Internal Links</td>
                                                </tr>
                                          </thead>
                                             <tbody>
                 <?php do { ?>
...
?>                 
                                                ...
                                                <tr>
                                                   <td><a href="<?php echo $row_selected['actual_url']; ?>" target="_blank" class="client-link" data-toggle="tab"><?php echo $row_selected['actual_url']; ?></a>
