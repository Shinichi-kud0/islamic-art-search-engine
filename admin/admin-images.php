...
mysqli_select_db($saha, $database_saha);$query_total = "SELECT * FROM crawl_images WHERE (crawl_images.image_url LIKE '%$cid%' OR crawl_images.current_url LIKE '%$cid%'  OR crawl_images.keywords LIKE '%$cid%'  OR crawl_images.actual_url LIKE '%$cid%')";
...
mysqli_select_db($saha, $database_saha);$query_selected = "SELECT * FROM crawl_images WHERE (crawl_images.image_url LIKE '%$cid%' OR crawl_images.current_url LIKE '%$cid%'  OR crawl_images.keywords LIKE '%$cid%' OR crawl_images.actual_url LIKE '%$cid%') LIMIT $stt,$lim";
$selected = mysqli_query($saha, $query_selected) or die(mysqli_error($saha));
$row_selected = mysqli_fetch_assoc($selected);
$totalRows_selected = mysqli_num_rows($selected);
...
mysqli_select_db($saha, $database_saha);$query_total = "SELECT * FROM crawl_images";
$total = mysqli_query($saha, $query_total) or die(mysqli_error($saha));
$row_total = mysqli_fetch_assoc($total);
$totalRows_total = mysqli_num_rows($total);


mysqli_select_db($saha, $database_saha);$query_selected = "SELECT * FROM crawl_images ORDER BY crawl_images.visits DESC LIMIT $stt,$lim";
$selected = mysqli_query($saha, $query_selected) or die(mysqli_error($saha));
$row_selected = mysqli_fetch_assoc($selected);
$totalRows_selected = mysqli_num_rows($selected);
...
<?php if((@$_GET['q'])!=""){?>                                            
<a href="admin-images.php" class="btn btn-warning btn-lg">Reset</a>
<?php }?>
</td>
                                            <td class="client-status" colspan="2"><a href="admin-images-new.php" class="btn btn-success btn-lg">Manual URL</a></td>
...
<?php do { ?>
                                                ...
                                                <tr>
                                                  <td><div style="width:150px; height:100px; overflow:hidden"><img src="piccrop.php?w=100&img=<?php echo base64_encode($row_selected['image_url']); ?>"></div></td>
                                                   <td><?php echo $row_selected['keywords']; ?>
                                                   
                                                   </td>
                                                   <td> <strong>Base URL: <?php echo $row_selected['base_url']; ?></strong><br>
                                                   <strong>URL :</strong>
<a href="<?php echo $row_selected['current_url']; ?>" target="_blank" class="client-link" data-toggle="tab"><?php echo $row_selected['current_url']; ?></a></td>
                                                   <td><?php echo $row_selected['visits']; ?>&nbsp;</td>
                                                   <td class="client-status"><a href="admin-images-update.php?id=<?php echo $row_selected['id']; ?>" class="btn btn-primary">Update</a></td>
                                                   <td class="client-status"> 
<?php if($row_selected['deleted']=="Y"){?>
<a href="admin-images-delete.php?id=<?php echo $row_selected['id']; ?>&deleted=N"  class="btn btn-warning">Un-Delete</a>
<?php }else{?>
<a href="admin-images-delete.php?id=<?php echo $row_selected['id']; ?>&deleted=Y"  class="btn btn-danger">Delete</a>
...
<?php } ?>                   
                                                <tr>
                                                  <td colspan="6"><?php if(@$totalRows_total>@$lim){?>
