...
mysqli_select_db($saha, $database_saha);$query_selected = "SELECT * FROM crawl WHERE crawl.id='$id'";
...
$count=0;
if($count<$BatchCount){
ImageCrawl($database_saha,$saha,$row_selected['current_url'],$ImageHeight,$ImageWidth,$BatchCount,$count,$row_selected['actual_url'],$MaxLinksPerSite);
$CrawlingCompleted="true";
}else{
$CrawlingCompleted="false";
}

mysqli_select_db($saha, $database_saha);$query_selectedCrawl = "SELECT * FROM crawl_images WHERE crawl_images.current_url= '$CURL' AND crawl_images.deleted='N'  AND crawl_images.block_update='N'";
...
$totalRows_selectedCrawl = mysqli_num_rows($selectedCrawl);
  ...
  $updateSQL = sprintf("UPDATE crawl SET crawlRunImages=%s WHERE id=%s",
                       GetSQLValueString($_SESSION['crawlRunCode'], "text"),
...
?><i class="fa fa-check"></i> <?php echo $totalRows_selectedCrawl; ?> Images Crawled Sucessfully !